...
export const getCoursesByLeaderId = async (leaderId: number) => {
  return await db.select().from(courses).where(eq(courses.leaderId, leaderId));
};
...
export const updateCourse = async (
  ...
  const existingCourse = await db
    ...
    .from(courses)
    .where(
      and(eq(courses.code, code.trim()), sql`${courses.id} != ${courseId}`),
    )
    .limit(1);
...
export const getMostRecentlyAccessedCourse = async (leaderId: number) => {
  ...
  const coursesWithAccess = userCourses.filter((course) => course.lastAccessed);
...
export const createSession = async (
  ...
  // Check if the current minute is in the last 15 minutes of the hour (45-59)
  const currentMinute = sessionDate.getMinutes();
  if (currentMinute >= 45) {
    return {
      ok: false,
      error: "Cannot start session now",
    };
  }
